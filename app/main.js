...
console.log(`krateZ@${app.getVersion()} { Electron: ${process.versions.electron}, Node: ${process.versions.node}, Chromium: ${process.versions.chrome} }`);
...
const userscriptsDir = PathUtils.isValidPath(userscriptsDirConfig) ? userscriptsDirConfig : path.join(app.getPath("documents"), "krateZ/scripts");
...
let init = function() {
	...
	app.once("ready", async() => {
		...
		protocol.registerFileProtocol("krateZ-swap", (request, callback) => callback(decodeURI(request.url.replace(/^krateZ-swap:/, ""))));
