...
class WindowManager {
	...
	constructor(callerId, hideKrunkerWindowsOnShow = true) {
		...
		document.addEventListener("DOMContentLoaded", () => {
			if (!document.getElementById("krateZ-windowHolder")) {
				let w = document.createElement("div");
				w.setAttribute("id", "krateZ-windowHolder");
				w.setAttribute("style", "display: none;");
				w.innerHTML = '<div id="krateZ-menuWindow"></div>';
				...
				document.getElementsByTagName("body")[0].addEventListener("click", e => {
					// @ts-ignore
					(!e.path.find(p => p.id === "krateZ-menuWindow" || p.id === this.callerId)) && this.hide();
				});
			}
		});
	}
	 ...
	 */
	setContent(content) {
		document.getElementById("krateZ-menuWindow").innerHTML = content;
	}
	...
	show() {
		...
		document.getElementById("krateZ-windowHolder").setAttribute("style", "display: block;");
		this.shown = true;
	}
	 ...
	 */
	hide() {
		document.getElementById("krateZ-windowHolder").setAttribute("style", "display: none;");
		this.shown = false;
	}
