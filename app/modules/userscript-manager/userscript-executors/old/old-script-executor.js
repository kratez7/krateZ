...
class OldScriptExecutor extends IScriptExecutor {
	...
	async executeScript() {
		...
		if (blockReason > 0) {
			...
			console.error(`[krateZ] Blocked Script: ${this.#script.name} by ${this.#script.author}. Reason: ${[reasonMapping[blockReason - 1]]}`);
			return false;
		}

		console.log(`[krateZ] Executing userscript: ${this.#script.name} by ${this.#script.author}`);
